...
const Router = appInstance => {
  appInstance.route('/api/:controller')
  ...
  appInstance.route('*')
    .get(({ isStaticFile }, response) => {
      if (isStaticFile) response.sendFile(path.join(isStaticFile))
      else response.sendFile(path.join(staticDirectoryPath + '/index.html'))
    })
